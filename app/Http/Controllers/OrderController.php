    ...
    public function save(){
        ...
        try{
            DB::transaction(function(){
                ...
                foreach (session('cart') as $varisantID => $item){
                    $totalAmount += $item['quantity'] * $item['price_sale'] ?: $item['price_regular'];
                    $dataItem = [
                                    'product_variant_id' => $varisantID,
            ...
            session()->forget('cart');
            return redirect()->route('welcome')->with('success', 'Đặng hàng thành công');
        }catch(\Exception $e){
            dd($e->getMessage());
            return back()->with('error', 'lỗi đặng hàng');
        }
    }
}
